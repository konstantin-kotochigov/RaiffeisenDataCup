...
showMap <- function(custid)
{
  ...
  bgMap = get_map(location=c(mean(x$pos_atm_lon,na.rm=T), lon = mean(x$pos_atm_lat,na.rm=T)), source = "google", zoom = 7, color="bw")
  ggmap(bgMap) + geom_point(aes(x = pos_atm_lon, y = pos_atm_lat), data = x, alpha = .75, size=3, color="red") + 
...
# Load data
  ...
  setwd("/Users/Konstantin/Downloads/")
...
# Lookup City Center
  ...
  cities.center.manual <- 
    raw.df[,
           .(
             center.manual.lat=mean(ifelse(is.na(pos_lat), atm_lat, pos_lat), na.rm=T), 
             center.manual.lon=mean(ifelse(is.na(pos_lon), atm_lon, pos_lon), na.rm=T)), 
           by=city]
  raw.df <- merge(raw.df, cities.center.manual, by.x="city", by.y="city", all.x=T, all.y=F)
  ...
  city.freq <- raw.df[!is.na(raw.df$pos_atm_lat) & !is.na(pos_atm_lon),.(n=.N),by=.(customer_id,city,city_lat,city_lon,city_pop)]
  city.freq <- city.freq[order(customer_id,-n),,]
  customer.top.city <- city.freq[,.(top_city=head(city,1), top_city_lat=head(city_lat,1), top_city_lon=head(city_lon,1), top_city_pop=head(city_pop,1)),by=.(customer_id)]
