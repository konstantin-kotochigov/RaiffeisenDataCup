# Optimize hyperparameters
  ...
  for (i in 1:nrow(catboost_param_grid))
  {
    ...
    for (cv in 1:cv_sets)
    {
      ...
      # Store performance for curent model fit
        ...
        cv_accuracy_home <- c(cv_accuracy_home, sum(pred_home$home_dist <= 0.02) / nrow(pred_home))
        cv_accuracy_work <- c(cv_accuracy_work, sum(pred_work$work_dist <= 0.02) / nrow(pred_work))
        cv_accuracy <- c(cv_accuracy, (sum(pred_home$home_dist <= 0.02) + sum(pred_work$work_dist <= 0.02)) / (nrow(pred_home) + nrow(pred_work)))
        best_iterations_home <- c(best_iterations_home, catboost_model_home_for_validation$tree_count)
        best_iterations_work <- c(best_iterations_work, catboost_model_work_for_validation$tree_count)
  ...
  for (i in 1:nrow(lightgbm_param_grid))
  {
    ...
    for (cv in 1:cv_sets)
    {
      ...
      dtrain_home <- lgb.Dataset(data = as.matrix(result_train_train_home[,predictors,with=F]), label = as.character(result_train_train_home$target_home), free_raw_data = FALSE)
      dtest_home <-  lgb.Dataset(data = as.matrix(result_train_validate_home[,predictors,with=F]), label = as.character(result_train_validate_home$target_home)-1, free_raw_data = FALSE)
      valids <- list(train = dtrain_home, test = dtest_home)
      ...
      lightgbm_model_work_for_validation <- lightgbm(
        data = result_train_train_work[,numerics,with=F], 
        label=result_train_train_work$target_home, 
        max_depth=10, 
        nrounds=10, 
        objective="binary", 
        nthread=12)
